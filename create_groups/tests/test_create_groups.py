...
from create_groups.parser import parser
random.seed(default_seed)

class TestCreateGroups(unittest.TestCase):
    def test_parse_args_1(self):
        patch_location = 'create_groups/tests/mock/patches'
        args_str = f"--patch_location {patch_location} --out_location . --subtypes MMRd=1 Other=0 p53abn=0 p53wt=0 POLE=0"
        config = parser.parse_args(args_str.split())
    ...
    def test_parse_args_2(self):
        patch_location = 'create_groups/tests/mock/patches'
        args_str = f"--patch_location {patch_location} --out_location . --subtypes MMRd=1 Other=0 p53abn=0 p53wt=0 POLE=0 --is_binary"
        config = parser.parse_args(args_str.split())
        gc = GroupCreator(config)
        assert gc.is_binary == True
        assert {c.name: c.value for c in gc.CategoryEnum} == {'Tumor': 1, 'Normal': 0}

    def test_get_patch_paths_1(self):
        ...
        args_str = f"--patch_location {patch_location} --patch_pattern {patch_pattern}  --out_location . --is_binary --is_multiscale"
        config = parser.parse_args(args_str.split())
        gc = GroupCreator(config)
        assert gc.patch_location == patch_location
        assert gc.patch_pattern == utils.create_patch_pattern(patch_pattern)
        assert gc.is_binary == True
        assert gc.is_multiscale == True
        assert {c.name: c.value for c in gc.CategoryEnum} == {'Tumor': 1, 'Normal': 0}
        patch_paths = gc.get_patch_paths()
        assert len(patch_paths) == 1728
    
    def test_get_patch_paths_2(self):
        """Test GroupCreator.get_patch_paths() filters patches
        """
        is_binary = True
        patch_location = 'create_groups/tests/mock/patches'
        patch_pattern = 'annotation/subtype/slide/patch_size/magnification'
        args_str = f"--patch_location {patch_location} --patch_pattern {patch_pattern}  --out_location . --is_binary --is_multiscale --filter_labels patch_size=256 magnification=10 annotation=Tumor"
        config = parser.parse_args(args_str.split())
        gc = GroupCreator(config)
        assert gc.filter_labels == {'patch_size': '256', 'magnification': '10', 'annotation': 'Tumor'}
        patch_paths = gc.get_patch_paths()
    ...
    def test_get_patch_paths_3(self):
        is_binary = True
        patch_location = 'create_groups/tests/mock/patches'
        patch_pattern = 'annotation/subtype/slide/patch_size/magnification'
        args_str = f"--patch_location {patch_location} --patch_pattern {patch_pattern}  --out_location . --is_binary --is_multiscale --filter_labels patch_size=512 magnification=10"
        config = parser.parse_args(args_str.split())
        gc = GroupCreator(config)
        assert gc.filter_labels == {'patch_size': '512', 'magnification': '10'}
        patch_paths = gc.get_patch_paths()
        patch_pattern = utils.create_patch_pattern(patch_pattern)
        CategoryEnum = utils.create_category_enum(is_binary)
    ...
    def test_get_patch_paths_4(self):
        is_binary = False
        patch_location = 'create_groups/tests/mock/patches'
        patch_pattern = 'annotation/subtype/slide/patch_size/magnification'
        args_str = f"--patch_location {patch_location} --patch_pattern {patch_pattern}  --out_location . --is_binary --is_multiscale --filter_labels subtype=p53abn"
        config = parser.parse_args(args_str.split())
        gc = GroupCreator(config)
        assert gc.filter_labels == {'subtype': 'p53abn'}
        patch_paths = gc.get_patch_paths()
        patch_pattern = utils.create_patch_pattern(patch_pattern)
    ...
    def test_select_patches_from_patient_1(self):
        args_str = """
            --patch_location .
            --out_location .
            --is_binary"""
    ...
    def test_select_patches_from_patient_2(self):
        args_str = """
            --patch_location .
            --out_location .
            --is_binary"""
    ...
    def test_select_patches_from_patient_3(self):
        args_str = """
            --patch_location .
            --out_location .
            --is_binary"""
    ...
    def test_create_patient_subtype_patch_to_select_count_1(self):
        args_str = """
            --patch_location .
            --out_location .
            --is_binary
    ...
    def test_create_patient_subtype_patch_to_select_count_2(self):
        args_str = """
            --patch_location .
            --out_location .
            --is_binary
    ...
    def test_create_patient_subtype_patch_to_select_count_3(self):
        args_str = """
            --patch_location .
            --out_location .
